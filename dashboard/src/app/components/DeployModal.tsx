...
import EnvConflictModal, { EnvConflictEntry } from "./EnvConflictModal";
...
export default function DeployModal({ onClose }: { onClose: (jobId?: string) => void }) {
  ...
  const [envConflict, setEnvConflict] = useState<{ isOpen: boolean, conflicts: EnvConflictEntry[] }>({ isOpen: false, conflicts: [] });
  ...
  const detectEnvConflicts = async (appId: string): Promise<EnvConflictEntry[]> => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`${API_BASE_URL}/apps/${appId}/detect-env-conflicts`, {
      ...
      console.error("Failed to detect env conflicts", err);
      return [];
    }
  };

  const handleDeploy = async (overridePort?: number, envPrecedence: "dashboard" | "repo" = "dashboard") => {
    ...
    try {
      ...
      // 3. Check for dashboard/repo env conflicts before deploy
      if (appId) {
        const conflicts = await detectEnvConflicts(appId);
        if (conflicts.length > 0 && envPrecedence === "dashboard") {
          setLoading(false);
          setEnvConflict({ isOpen: true, conflicts });
          return;
        }
      }

      // 4. If an override port was chosen, update the app first
      if (isOverride && appId) {
      ...
      // 5. Trigger actual deployment
      const deployRes = await fetch(`${API_BASE_URL}/apps/${appId}/deploy?trigger_reason=Manual:Canvas&env_precedence=${envPrecedence}`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${session?.access_token}`,
        }
  ...
  return (
    ...
    >
      <div className="bg-card border border-card-border w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden cursor-default flex flex-col h-[85vh] animate-in zoom-in-95 duration-200">
        <EnvConflictModal
          isOpen={envConflict.isOpen}
          onClose={() => setEnvConflict({ isOpen: false, conflicts: [] })}
          onConfirm={(choice) => {
            setEnvConflict({ isOpen: false, conflicts: [] });
            handleDeploy(undefined, choice);
          }}
          conflicts={envConflict.conflicts}
        />
