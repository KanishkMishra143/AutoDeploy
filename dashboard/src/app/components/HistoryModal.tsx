...
export default function AppDetailModal({ app: initialApp, onClose, onViewLogs, allJobs, allApps, credentials }: AppDetailModalProps) {
  ...
  const [envConflict, setEnvConflict] = useState<{ isOpen: boolean; conflicts: EnvConflictEntry[] }>({
    isOpen: false,
    conflicts: []
  });
  ...
  const detectEnvConflicts = async (): Promise<EnvConflictEntry[]> => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`${API_BASE_URL}/apps/${app.id}/detect-env-conflicts`, {
        headers: { "Authorization": `Bearer ${session?.access_token}` }
      });
      if (!res.ok) return [];
      const data = await res.json();
  ...
  const handleDeploy = async (overridePort?: number, envPrecedence: "dashboard" | "repo" = "dashboard") => {
    ...
    try {
      ...
      // 2. Check env conflicts before deployment
      const conflicts = await detectEnvConflicts();
      ...
      const res = await fetch(`${API_BASE_URL}/apps/${app.id}/deploy?trigger_reason=Manual:Canvas&env_precedence=${envPrecedence}`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${session?.access_token}`,
        }
